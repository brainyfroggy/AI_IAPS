#!/usr/bin/env python3
"""Run decode_roi_singletrial.py across the full cohort with bounded parallelism."""
...
def run_one(subject: int) -> dict:
    ...
    log_path = ROOT / "logs" / f"decode_roi_sub{subject:02d}.log"
...
def main() -> int:
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(run_one, s): s for s in args.subjects}
    ...
    print(f"DECODE_WAVE_COMPLETE total={len(results)} failed={n_fail}", flush=True)
