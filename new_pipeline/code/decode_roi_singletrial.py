...
Copied from sub4_spatial_sensitivity_32/code/decode_roi_singletrial.py (that study's
completed artifact is left untouched) with one generalization: --subject replaces the
hardcoded "Sub4" in the output metadata so this can run across the full cohort. No other
logic changed -- same 17 ROIs, same 8 within/cross-source contrasts, same linear-SVM
leave-one-run-out decoding, same mask-normalized post-GLM smoothing.
"""
...
def main() -> None:
    ...
    parser.add_argument("--subject", required=True, help="e.g. Sub01")
    parser.add_argument("--estimator", choices=["glmsingle_typed"], default="glmsingle_typed")
    ...
    print(f"ROI_SINGLETRIAL_COMPLETE {args.subject} {args.estimator} {args.branch} {args.smoothing_mm:g}mm")
