#!/usr/bin/env python3
"""Reproduce the Bo et al.-style within/cross source bar charts (as used in
kastner_singletrial_top50_decoding.py's plot_same_style) for the GLMsingle Random
10-fold x30 single-trial decoding results (roi_decoding_random10fold/group/group_roi_stats.csv).
"""
...
GROUP_CSV = NP / "roi_decoding_random10fold" / "group" / "group_roi_stats.csv"
...
def add_guide_lines(ax, low: float, high: float, step: float = 0.02) -> None:
    v = low
    while v <= high + 1e-9:
...
def plot_panel(df: pd.DataFrame, comp_order: list[str], colors: dict, labels: dict,
    ...
    # bars here reach the axis floor, so a lower-right legend would sit on top of the
    # PHC bars; upper-left is empty in both panels at this accuracy range.
...
def main() -> None:
    ...
    # single-trial accuracies span a much narrower band than the ERP-style reference
    # figure (which ran 40-85%), so the axis is tightened to keep the effects readable.
    y_low, y_high, step = 0.45, 0.60, 0.025

    plot_panel(df, WITHIN_ORDER, WITHIN_COLORS, WITHIN_LABELS,
              "Within Source Decoding (GLMsingle, Random 10-fold x30)",
              OUT_DIR / "within_kastner_wang_random10fold_glmsingle.png", y_low, y_high, step)
    plot_panel(df, CROSS_ORDER, CROSS_COLORS, CROSS_LABELS,
              "Cross Source Decoding (GLMsingle, Random 10-fold x30)",
              OUT_DIR / "cross_kastner_wang_random10fold_glmsingle.png", y_low, y_high, step)
