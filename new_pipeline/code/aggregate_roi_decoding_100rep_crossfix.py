#!/usr/bin/env python3
"""Aggregate the corrected (crossfix) per-subject decoding results and add group-level
statistics. Within-source contrasts: random 10-fold CV x100 repeats (Bo et al. 2021
scheme, unchanged). Cross-source contrasts: single train-on-all-200/test-on-all-200
split, no CV (corrected design -- see decode_roi_random10fold.py). Mirrors
aggregate_roi_decoding_random10fold_100rep.py exactly, pointed at the merged/fixed tree.
"""
...
ROOT = Path("/mnt/n/Experimental_Data/yujunchen/projects/AI_IAPS/new_pipeline/roi_decoding_random10fold_100rep_fixed")
OUT = ROOT / "group"
...
def main() -> None:
    ...
    print(f"cross-source grand mean (crossfix): {cross_mean*100:.2f}%")
    print(f"overall grand mean (within+cross): {grand_mean*100:.2f}%")

    chart_data = {
        "n_subjects": int(n_subjects),
        "chance": CHANCE,
        "cv_scheme": ("within-source: random 10-fold x100 repeats (Bo et al. 2021 scheme); "
                      "cross-source: single train-on-all-200/test-on-all-200 split, no CV (corrected)"),
