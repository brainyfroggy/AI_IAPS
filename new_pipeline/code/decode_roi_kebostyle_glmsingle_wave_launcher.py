#!/usr/bin/env python3
"""Run decode_roi_kebostyle_glmsingle.py across the full cohort with bounded parallelism.

Two levels of concurrency: --workers subjects run concurrently, each internally using
--inner-jobs ROI x contrast workers (see kebo_decode_core.py). Keep workers * inner_jobs
close to the machine's core count without exceeding it.
"""
...
SCRIPT = ROOT / "code" / "decode_roi_kebostyle_glmsingle.py"
...
def run_one(subject: int, inner_jobs: int, out_root: str) -> dict:
    ...
    log_path = ROOT / "logs" / f"decode_roi_kebostyle_glmsingle_sub{subject:02d}.log"
...
def main() -> int:
    ...
    ap.add_argument("--output-root", default=str(ROOT / "roi_decoding_kebostyle_glmsingle"))
    args = ap.parse_args()
    ...
    print(f"KEBOSTYLE_GLMSINGLE_WAVE_COMPLETE total={len(results)} failed={n_fail}", flush=True)
