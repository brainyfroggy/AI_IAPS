#!/usr/bin/env python3
"""Aggregate per-subject Kastner ROI decoding results (n=29) and add group-level statistics.

The single-subject decoder (decode_roi_singletrial.py, adapted from the Sub4 spatial-
sensitivity study) computes no significance testing at all -- just point-estimate LORO
accuracy per subject/ROI/contrast. This script adds the group layer: for each of the
17 ROIs x 8 contrasts, a one-sample t-test of the 29 subjects' accuracies against chance
(50%), with Benjamini-Hochberg FDR correction applied within each contrast across its
17 ROIs (matching the FDR convention used elsewhere in this project's group-level maps).
"""
...
ROOT = Path("/mnt/n/Experimental_Data/yujunchen/projects/AI_IAPS/new_pipeline/roi_decoding")
OUT = ROOT / "group"
