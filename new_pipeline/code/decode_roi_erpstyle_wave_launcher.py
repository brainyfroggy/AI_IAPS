#!/usr/bin/env python3
"""Run decode_roi_erpstyle.py across the full cohort with bounded parallelism."""
...
def run_one(subject: int, out_root: str) -> dict:
    ...
    log_path = ROOT / "logs" / f"decode_roi_erpstyle_sub{subject:02d}.log"
...
def main() -> int:
    ...
    ap.add_argument("--output-root", default=str(ROOT / "roi_decoding_erpstyle"))
    args = ap.parse_args()

    results = []
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(run_one, s, args.output_root): s for s in args.subjects}
    ...
    print(f"ERPSTYLE_WAVE_COMPLETE total={len(results)} failed={n_fail}", flush=True)
