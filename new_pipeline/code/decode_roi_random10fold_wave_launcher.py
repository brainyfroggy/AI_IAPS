#!/usr/bin/env python3
"""Run decode_roi_random10fold.py across the full cohort with bounded parallelism."""
...
SCRIPT = ROOT / "code" / "decode_roi_random10fold.py"
...
def run_one(subject: int, n_repeats: int, out_root: str) -> dict:
    ...
    log_path = ROOT / "logs" / f"decode_roi_random10fold_sub{subject:02d}.log"
...
def main() -> int:
    ...
    ap.add_argument("--output-root", default=str(ROOT / "roi_decoding_random10fold"))
    args = ap.parse_args()
    ...
    print(f"RANDOM10FOLD_WAVE_COMPLETE total={len(results)} failed={n_fail}", flush=True)
