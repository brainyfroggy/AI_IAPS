#!/usr/bin/env python3
"""Aggregate per-subject Ke Bo-style single-trial Kastner ROI decoding results (either
GLMsingle or SPM beta source) and add group-level statistics. Mirrors
aggregate_roi_decoding_random10fold.py's pattern.

Usage: aggregate_roi_decoding_kebostyle.py <root> [--exclude-subjects Sub3 Sub8]
"""
...
def main() -> None:
    ...
    ap.add_argument("--cv-label", default="Ke Bo scheme: StratifiedKFold(4) x 20 reps, StandardScaler(train-only), LinearSVC")
    args = ap.parse_args()
    ...
    n_subjects_all = all_results["subject"].nunique()
    print(f"aggregated {len(all_results)} rows across {n_subjects_all} subjects (all, before group-exclusion)")

    group_results = all_results[~all_results["subject"].isin(args.exclude_subjects)]
    n_subjects = group_results["subject"].nunique()
    print(f"group stats computed on {n_subjects} subjects (excluded: {args.exclude_subjects})")

    rows = []
    for contrast in CONTRAST_ORDER:
        sub_frame = group_results[group_results["contrast"] == contrast]
        pvals = []
        cell_data = []
