...
cwd = 'F:\yujun\projects\LAB_IAPS_AI\prepfMRI\testsession\subject1\Run1\';   % Working directory
   
behavdir = 'C:\Users\Vladimir Liu\MyProjects\EEG_fMRI\Conditioning\BehavioralData\Sub16_Run1.mat';      % File Directory for the stimulus onset timings
        
% sessionID = 1;                   % The session number from 1 -> 3; change it accordingly
ncon = 6;                        % Number of Conditions you wish to model: CS+, CS-
cname = cell(1,6); 
cname{1,1} = 'nonAI_pl'; 
cname{1,2} = 'nonAI_nt'; 
cname{1,3} = 'nonAI_up'; 
cname{1,4} = 'AI_pl'; 
cname{1,5} = 'AI_nt'; 
cname{1,6} = 'AI_up';          % Name of each condition; make sure the order is the same with what's stored in 'sots.mat';

nsess = 1;                       % Total number of sessions/runs to be modeled, 3 in this case, habituation, acquisition, and extinction; each modeled separately
TR = 1.8;

nscan = 224;           % Number of scans for each of the three sessions
...
SPM.nscan = ones(1,nses)*224;    % Get the # of scans within the current session to be analyzed
...
SPM.xBF.T = 64;              % Number of slices. This goes into the Microtime Resolution
SPM.xBF.T0 = 32;             % Middle slice as the reference slice. This goes into the Microtime Onset
SPM.xBF.UNITS = 'secs';     % OPTIONS: 'scans'|'secs'
...
for c = 1:ncon(ses)
    ...
    SPM.Sess(1).U(c).dur = 3;                  % presentation time, 3 secs
...
tmp{1} = spm_select('fplist',cwd,'^swas.*\.img');
