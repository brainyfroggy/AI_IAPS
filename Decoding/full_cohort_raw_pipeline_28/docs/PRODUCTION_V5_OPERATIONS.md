# AI-IAPS remaining-cohort production v5 operations record
...
incident-specific explanations are in
`audit/PRODUCTION_V4_PATH_RECOVERY_20260730.md` and
`audit/PRODUCTION_V4_BRANCH_IDENTITY_FAILURE_20260730.md`.
...
These are the compute settings that worked for Subjects 4-6. V5 retains the
corrected host path boundary and storage policy and canonicalizes the persisted
SDC branch identity; it does not change these compute settings.
...
- Immediately before the v5 freeze C: had approximately 59.22 GiB free; N: had
  17.83 TiB free.
...
- Analysis ID: `ai_iaps_full28_sdc_glmsingle_erp_v5`.
- Freeze: `ANALYSIS_FREEZE_v5.json`, SHA-256
  `2d266d3c2e18bdfe375b2495e060c3185a6e68fe338971f728611394c7897923`.
- Stage contract: `config/stage_contracts_v5.json`, SHA-256
  `4dbd71a3fccba30354c57c32651490fbdaacb5da43bbf5d38f77453324094176`.
- Production approval: `runtime/production_approval_v5.json`, SHA-256
  `e19f79d60a5d7bdc6083e19fb45e0b85c5000c0cdf4f02acc737e5b9898c325e`.
...
- Hash-chained ledger: `logs/stage_attempts_v5/ledger.jsonl`.
- Batch stdout/stderr: `logs/batch_v5/runner.stdout.log` and
  `logs/batch_v5/runner.stderr.log`.
...
- Canonical BIDS inputs: `runtime/production_v3/bids/SubXX`. This namespace is
  ...
  stop. V5 adopts an existing subject only after complete byte verification;
  later subjects are built by the same verified route.
- V5 fMRIPrep derivatives: `runtime/production_v5/fmriprep/SubXX`.
- V5 isolated SDC branch inputs and outputs:
  `runtime/production_v5/bids_branches/SubXX` and
  `runtime/production_v5/fmriprep_branch_outputs/SubXX`.
- V5 GLMsingle: `derivatives/production_v5/glmsingle/SubXX`.
- V5 ERP subject results: `results/production_v5/erp_subjects/SubXX`.
- V5 audit/QC/archives: `audit/production_v5/...`.
- V5 immutable attempts: `logs/stage_attempts_v5/subjects/SubXX/stages/...`.
...
& $PY .\code\run_remaining_cohort.py `
  --freeze .\ANALYSIS_FREEZE_v5.json `
  --contract .\config\stage_contracts_v5.json `
  --approval .\runtime\production_approval_v5.json `
  --log-root .\logs\stage_attempts_v5 `
  --execute `
  --ack-analysis-id ai_iaps_full28_sdc_glmsingle_erp_v5
```
...
```powershell
Get-Content .\logs\batch_v5\runner.stdout.log -Tail 30
Get-Content .\logs\batch_v5\runner.stderr.log -Tail 30
Get-Content .\logs\stage_attempts_v5\ledger.jsonl -Tail 10
...
4. Correct only the documented cause. If any pinned file must change, create a
   new frozen control version; never mutate the v5 approval.
...
- 37 production-control/SDC tests passed.
- 34 analysis/postprocessing tests passed.
- The JSON-persisted SDC branch identity passed an explicit round-trip
  regression test before the v5 freeze.
